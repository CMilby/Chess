function rookAllMoves(
  ...
  for (let i = x + 1; i < 8; i++) {
    let canMove = rookCanMoveTo(i, y, board, color);
  ...
  canMoveMore = true;
  for (let i = y + 1; i < 8; i++) {
    let canMove = rookCanMoveTo(x, i, board, color);
    if (canMove >= condition) {
      moves.push([x, i]);
    }
...
export function rookPossibleMoves(
  x: number,
  y: number,
  board: {
    piece: string;
  ...
  let allMoves = rookAllMoves(x, y, board, color, 1);
...
export function rookCoveredSquares(
  x: number,
  y: number,
  board: {
    piece: string;
  ...
  return rookAllMoves(x, y, board, color, 0);
}

function rookCanMoveTo(
  toX: number,
  toY: number,
  board: {
    piece: string;
